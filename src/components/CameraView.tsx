...
import { Camera, RefreshCw, Sparkles, User, AlertTriangle, Timer, Disc, LayoutGrid, Image as ImageIcon } from 'lucide-react';
...
export const FUNNY_EFFECTS: FunnyEffect[] = [
  ...
  {
    id: 'fisheye_selfie',
    name: '0.5 Ultra-wide Fisheye Selfie 📸',
    emoji: '👁️',
    cssFilter: 'saturate(1.22) contrast(1.06) brightness(1.02)',
    canvasFilter: 'saturate(122%) contrast(106%) brightness(102%)',
    description: 'Sensasi kamera ultra-wide 0.5x cembung fisheye estetik ala gen-z'
  },
  {
    id: 'kirakira',
...
export default function CameraView({
  ...
  useEffect(() => {
    ...
    async function startCamera() {
      try {
        ...
        if (isCurrent) {
          console.warn('Gagal mengakses kamera (mengaktifkan simulator):', err);
  ...
  const handleLocalFilesUploadInCamera = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files;
    if (!files || files.length === 0) return;

    setIsCapturing(true);
    const loadedPhotos: string[] = [];
    const totalExpected = captureMode === 'single' ? 1 : 4;

    try {
      for (let i = 0; i < Math.min(files.length, totalExpected); i++) {
        const file = files[i];
        ...
        loadedPhotos.push(dataUrl);
      }

      if (loadedPhotos.length > 0) {
        while (loadedPhotos.length < totalExpected) {
          loadedPhotos.push(loadedPhotos[loadedPhotos.length % loadedPhotos.length]);
        }
        
        playSuccessChime();
        setCapturedList(loadedPhotos);
        setCapturedCount(loadedPhotos.length);
  ...
  const renderFunnyOverlayLive = () => {
    switch (funnyEffectId) {
      case 'fisheye_selfie':
        return (
          <div className="absolute inset-0 flex flex-col justify-between items-center p-4 font-mono text-white/70 text-[10px] select-none tracking-wider pointer-events-none z-20">
            {/* Top row with a glowing green active dot & 0.5X badge */}
            <div className="w-full flex justify-between items-center">
              <div className="flex items-center gap-1 bg-black/40 px-2 py-1 rounded-full border border-white/5 backdrop-blur-sm">
                ...
                <span className="font-extrabold text-[8px] uppercase text-emerald-400">0.5X FISHEYE MODE</span>
              </div>
              <div className="bg-black/40 px-2 py-1 rounded-full border border-white/5 backdrop-blur-sm font-bold text-[8px]">
                SHUTTER SPEED: AUTO
              </div>
            </div>

            {/* Subtle retro target crosshair in the center */}
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-20">
              <div className="w-40 h-40 border border-dashed border-white rounded-full flex items-center justify-center">
                <div className="w-2 h-2 bg-white rounded-full" />
              </div>
            </div>

            {/* Cute sparkles floating in top corners */}
            <div className="absolute top-[18%] left-[10%] text-3xl select-none animate-pulse z-10">✨</div>
            <div className="absolute top-[18%] right-[10%] text-3xl select-none animate-pulse z-10">✨</div>

            {/* Bottom active badge */}
            <div className="mt-auto bg-black/55 text-white font-extrabold px-3 py-1 rounded-full text-[9px] border border-white/10 uppercase tracking-widest backdrop-blur-md shadow-lg animate-bounce">
              🤳 Gen-Z FishEye Selfie Active
            </div>
          </div>
        );
      case 'kirakira':
  ...
  const drawFunnyOverlayToCanvas = (
    ...
    switch (effectId) {
      case 'fisheye_selfie':
        // A premium vintage/gen-z ultra-wide lens style indicator or small sparkles/stars for selfie
        ctx.save();
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        
        // Draw a futuristic/retrofuturistic lens crosshair or circular markings!
        ctx.strokeStyle = 'rgba(255, 255, 255, 0.15)';
        ctx.lineWidth = 1.5;
        
        // Draw subtle fish-eye circle markings
        ctx.beginPath();
        ...
        ctx.fillText('0.5x FISHEYE ULTRA-WIDE', width / 2, height - 80);
        
        // Some cute sparkles
        drawEmoji('✨', width * 0.18, height * 0.22, 45, 12);
        drawEmoji('✨', width * 0.82, height * 0.22, 45, -12);
        ctx.restore();
        break;
  ...
  const captureVideoFrame = (): string => {
    ...
    if (ctx) {
      ...
      if (lensType === '0.5x' || funnyEffectId === 'fisheye_selfie') {
      ...
      if (isCembung || funnyEffectId === 'fisheye_selfie') {
        applyCembungDistortion(ctx, canvas.width, canvas.height);
      }

      // Apply funny props overlays (drawn on top of the image)
      drawFunnyOverlayToCanvas(ctx, canvas.width, canvas.height, funnyEffectId, lensType === '0.5x' || funnyEffectId === 'fisheye_selfie');
        ...
        reader.onloadend = () => {
          ...
          rawImg.onload = () => {
            ...
            if (ctx) {
              ...
              // Apply funny props overlays (drawn on top of the image)
              drawFunnyOverlayToCanvas(ctx, 800, 1000, funnyEffectId, lensType === '0.5x' || funnyEffectId === 'fisheye_selfie');
        ...
        >
          ...
          <video
            ...
            style={{
              ...
              transform: `${facingMode === 'user' ? 'scaleX(-1)' : 'scaleX(1)'} ${(lensType === '0.5x' || funnyEffectId === 'fisheye_selfie') ? 'scale(0.72)' : 'scale(1)'} ${(isCembung || funnyEffectId === 'fisheye_selfie') ? 'scale(1.15)' : ''}`.trim(),
          ...
          {useSimulation && (
            <div className="w-full h-full relative bg-zinc-900 flex items-center justify-center overflow-hidden">
              <img
                ...
                style={{
                  filter: `${activeFilter.cssFilter !== 'none' ? activeFilter.cssFilter : ''} ${FUNNY_EFFECTS.find(e => e.id === funnyEffectId)?.cssFilter || ''}`.trim() || 'none',
                  transform: `${(lensType === '0.5x' || funnyEffectId === 'fisheye_selfie') ? 'scale(0.72)' : 'scale(1)'} ${(isCembung || funnyEffectId === 'fisheye_selfie') ? 'scale(1.15)' : ''}`.trim(),
                  transition: 'transform 0.4s cubic-bezier(0.2, 0.8, 0.2, 1)',
                  transformOrigin: 'center center',
                }}
                className="w-full h-full object-cover"
          ...
          <div
            ...
            style={{
              transform: (lensType === '0.5x' || funnyEffectId === 'fisheye_selfie') ? 'scale(0.72)' : 'scale(1)',
              transition: 'transform 0.4s cubic-bezier(0.2, 0.8, 0.2, 1)',
              transformOrigin: 'center center',
            }}
          >
          ...
          {(lensType === '0.5x' || funnyEffectId === 'fisheye_selfie') && (
          ...
          {(isCembung || funnyEffectId === 'fisheye_selfie') && (
          ...
          {permissionState === 'denied' && !useSimulation && (
            <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center bg-zinc-950 z-30">
              <AlertTriangle className="text-pink-400 mb-2 animate-bounce" size={32} />
              <p className="text-xs font-semibold text-zinc-200">Kamera Tidak Dapat Diakses</p>
              <p className="text-[10px] text-zinc-400 mt-1 mb-4 leading-relaxed px-2">
                Kamera tidak didukung browser / izin diblokir. Gunakan kamera simulasi atau langsung unggah foto pilihanmu!
              </p>
              <div className="flex flex-col gap-2 w-full max-w-[220px]">
                <button
                  onClick={() => setUseSimulation(true)}
                  className="text-xs bg-pink-500 hover:bg-pink-400 text-white font-bold py-2 px-4 rounded-xl transition cursor-pointer"
                >
                  Gunakan Kamera Virtual
                </button>
                <label className="text-xs bg-zinc-800 hover:bg-zinc-750 text-zinc-200 font-bold py-2 px-4 rounded-xl transition cursor-pointer flex items-center justify-center gap-1 border border-zinc-700">
                  <ImageIcon size={12} />
                  Unggah Foto dari HP
                  <input
                    type="file"
                    ...
                    accept="image/*"
                    className="hidden"
                    onChange={handleLocalFilesUploadInCamera}
                  />
                </label>
              </div>
            </div>
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/95 via-black/80 to-transparent pt-14 pb-3.5 px-3 flex flex-col gap-2.5 z-20 pointer-events-auto">
            ...
            <div className="flex items-center justify-between px-1 pt-2 border-t border-zinc-900/30">
              ...
              <label
                className={`p-2 rounded-full bg-zinc-900/60 hover:bg-zinc-800 text-zinc-400 hover:text-white transition-all duration-200 active:scale-95 disabled:opacity-30 cursor-pointer flex items-center justify-center`}
